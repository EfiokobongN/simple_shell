...
/**
 * _builtins - checks if command is builtin
 * @inputs: variables
...
/**
 * _env - current environment prints
...
void _unsetenv(input_t *inputs)
{
	...
	{
		_error(inputs, ": The number of arguments provided is incorrect\n");
		inputs->status = 2;
		return;
	}
	env = find_env(inputs->env, inputs->tokens[1]);
	if (env == NULL)
	{
		_error(inputs, ": There are no variables to unset");
		return;
	}
